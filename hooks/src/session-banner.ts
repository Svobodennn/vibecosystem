/**
 * SessionStart Hook: vibecosystem Banner
 *
 * Prints the ASCII banner with live component counts, mature instinct
 * patterns and recent team errors (canavar ledger) at session start.
 * Output uses systemMessage so the banner is visible in the session.
 */
...
function countFiles(dir: string, ext: string): number | string {
  try { return readdirSync(dir).filter(f => f.endsWith(ext)).length; } catch { return '?'; }
}

function countDirs(dir: string): number | string {
  try {
...
function boxLine(str: string): string {
  return `  │ ${pad(str, BOX_W - 6)} │`;
}

function loadPatterns(): Instinct[] {
  const maturePath = join(homedir(), '.claude', 'mature-instincts.json');
  if (!existsSync(maturePath)) return [];
  try {
    const instincts: Instinct[] = JSON.parse(readFileSync(maturePath, 'utf-8'));
    return instincts
...
function loadTeamErrors(): GroupedError[] {
