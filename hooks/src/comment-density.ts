// Comment Density Hook (PostToolUse:Edit|Write|MultiEdit) — reminds me when an
// edit adds verbose comments (JSDoc blocks, long comment runs, high ratio).
// Enforces WORKING_AGREEMENT rule 6 / the comment-style memory at write-time.
...
const cont = () => console.log(JSON.stringify({ result: "continue" }));
...
function main() {
  ...
  const dense =
    commentLines >= 4 && codeLines > 0 && commentLines / codeLines > 0.35;
