---
name: sentinel
description: "USE WHEN: SRE/on-call disiplini — service health monitoring, SLO error budget, alert routing+escalation, observability stack tasarımı, on-call rotation, runbook'ları sürekli güncelleme. NOT FOR: aktif P0-P3 incident yönetimi tek seferlik, metric DSL yazımı (PromQL), distributed trace impl, chaos experiment tasarımı. USE INSTEAD: incident-responder (aktif incident), prometheus-expert (PromQL+SLI/SLO impl), tracing-expert (OpenTelemetry), chaos-engineer (resilience test)."
...
```bash
git add -A
git commit -m "sentinel: <kisa degisiklik ozeti>" && echo COMMITTED || echo NO_CHANGES
