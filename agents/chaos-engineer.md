---
name: chaos-engineer
description: "USE WHEN: chaos experiment tasarımı (network partition, latency injection, pod kill), gameday planlama, blast radius kontrolü, steady-state hypothesis, resilience validation pre-prod/prod. NOT FOR: aktif incident response, load testing, generic monitoring, post-incident analiz. USE INSTEAD: incident-responder (aktif), load-tester (yük testi), sentinel (monitoring), coroner (post-mortem)."
...
```bash
git add -A
git commit -m "chaos-engineer: <kisa degisiklik ozeti>" && echo COMMITTED || echo NO_CHANGES
