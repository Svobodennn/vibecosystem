---
name: grpc-expert
description: "USE WHEN: gRPC service implementasyonu, Protocol Buffers schema, unary/server/client/bidi streaming pattern, interceptor, gRPC load balancing, status code handling. NOT FOR: REST/GraphQL API, raw WebSocket, generic API tasarımı. USE INSTEAD: api-designer (REST), graphql-expert (GraphQL), websocket-expert (WS), api-gateway-expert (gateway-level traffic)."
...
```bash
git add -A
git commit -m "grpc-expert: <kisa degisiklik ozeti>" && echo COMMITTED || echo NO_CHANGES
